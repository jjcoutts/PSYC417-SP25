...
iterate_mean <- function(data){ # similar to HW1
	tot = 0; n = 0
...
```{r}
iterate_mean(1:100);vectorized_mean(1:100)
iterate_mean(1:100000);vectorized_mean(1:100000)
iterate_mean(1:100000000);vectorized_mean(1:100000000)
```
...
vmax2(c(1:10)) # wwrong because missing argument
vmax2(10, c(1:10)) # fine
vmax2(c(1:10), 10) # wrong because arugments out of order and not named
```
...
```{r}
mean_choice <- function(data, type, expression){ # start function) 
	if(type == "iterate"){
		avg <- iterate_mean(data)
	} else if(type == "vectorized"){
		avg <- vectorized_mean(data)
	} else if(type == "baseR"){
		avg <- mean(data)
	}
